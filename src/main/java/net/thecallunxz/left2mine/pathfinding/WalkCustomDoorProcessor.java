...
public class WalkCustomDoorProcessor extends NodeProcessor {
   ...
   public PathPoint getStart() {
      ...
      if (this.getCanSwim() && this.entity.isInWater()) {
         ...
         MutableBlockPos blockpos$mutableblockpos = new MutableBlockPos(MathHelper.floor(this.entity.posX), i, MathHelper.floor(this.entity.posZ));
      ...
      for(int i = 0; i < xSize; ++i) {
         for(int j = 0; j < ySize; ++j) {
            for(int k = 0; k < zSize; ++k) {
               ...
               if (pathnodetype == PathNodeType.RAIL && !(p_193577_1_.getBlockState(p_193577_12_).getBlock() instanceof BlockNoPath) && !(p_193577_1_.getBlockState(p_193577_12_).getBlock() instanceof BlockRailBase) && !(p_193577_1_.getBlockState(p_193577_12_.down()).getBlock() instanceof BlockRailBase) && !(p_193577_1_.getBlockState(p_193577_12_.down()).getBlock() instanceof BlockNoPath)) {
   ...
   protected PathNodeType getPathNodeTypeRaw(IBlockAccess p_189553_1_, int p_189553_2_, int p_189553_3_, int p_189553_4_) {
      ...
      if (type != null) {
         ...
         } else if (block instanceof BlockGameDoor && material == Material.WOOD && !BlockGameDoor.isOpen(p_189553_1_, blockpos)) {
            return PathNodeType.DOOR_WOOD_CLOSED;
         } else if (block instanceof BlockGameDoor && material == Material.IRON && !BlockGameDoor.isOpen(p_189553_1_, blockpos)) {
            return PathNodeType.DOOR_IRON_CLOSED;
         } else if (block instanceof BlockGameDoor && BlockGameDoor.isOpen(p_189553_1_, blockpos)) {
            return PathNodeType.DOOR_OPEN;
         } else if (!(block instanceof BlockRailBase) && !(block instanceof BlockNoPath)) {
            if (block instanceof BlockFence || block instanceof BlockWall || block instanceof BlockFenceGate && !(Boolean)iblockstate.getValue(BlockFenceGate.OPEN)) {
               return PathNodeType.FENCE;
