...
public class BlockSurvivalSpawn extends BlockNodeParent {
   public BlockSurvivalSpawn(String name) {
      super(name);
   }
   ...
   public boolean canPlaceBlockAt(World worldIn, BlockPos pos) {
      if (super.canPlaceBlockAt(worldIn, pos)) {
         if (worldIn.getBlockState(pos).getBlock() == this) {
            ...
            if (data.isSurvivalSpawnSet(worldIn)) {
               return false;
            } else {
               data.setSurvivalSpawnPos(pos);
