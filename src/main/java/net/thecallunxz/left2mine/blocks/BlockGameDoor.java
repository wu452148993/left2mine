...
public class BlockGameDoor extends BlockBase implements ITileEntityProvider {
   public static final PropertyDirection FACING;
   public static final PropertyBool OPEN;
   public static final PropertyEnum<BlockGameDoor.EnumHingePosition> HINGE;
   public static final PropertyBool POWERED;
   public static final PropertyEnum<BlockGameDoor.EnumDoorHalf> HALF;
   ...
   public BlockGameDoor(Material mat, String name) {
      super(mat, name);
      this.setBlockUnbreakable();
      this.setResistance(6000001.0F);
      this.hasTileEntity = true;
      this.setDefaultState(this.blockState.getBaseState().withProperty(FACING, EnumFacing.NORTH).withProperty(OPEN, false).withProperty(HINGE, BlockGameDoor.EnumHingePosition.LEFT).withProperty(POWERED, false).withProperty(HALF, BlockGameDoor.EnumDoorHalf.LOWER));
   }

   public BlockGameDoor(String name) {
      this(Material.WOOD, name);
   }

   public BlockGameDoor(Material mat, String name, boolean wall) {
      super(mat, name);
      this.setBlockUnbreakable();
      this.setResistance(6000001.0F);
   }
   ...
   public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos) {
      ...
      boolean flag1 = state.getValue(HINGE) == BlockGameDoor.EnumHingePosition.RIGHT;
      switch(enumfacing) {
      case EAST:
      default:
   ...
   public MapColor getMapColor(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
      if (state.getBlock() == Blocks.IRON_DOOR) {
         ...
         return EnumType.OAK.getMapColor();
      } else if (state.getBlock() == Blocks.SPRUCE_DOOR) {
         return EnumType.SPRUCE.getMapColor();
      } else if (state.getBlock() == Blocks.BIRCH_DOOR) {
         return EnumType.BIRCH.getMapColor();
      } else if (state.getBlock() == Blocks.JUNGLE_DOOR) {
         return EnumType.JUNGLE.getMapColor();
      } else if (state.getBlock() == Blocks.ACACIA_DOOR) {
         return EnumType.ACACIA.getMapColor();
      } else {
         return state.getBlock() == Blocks.DARK_OAK_DOOR ? EnumType.DARK_OAK.getMapColor() : super.getMapColor(state, worldIn, pos);
      }
   }

   public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
      BlockPos blockpos = state.getValue(HALF) == BlockGameDoor.EnumDoorHalf.LOWER ? pos : pos.down();
      ...
      if (iblockstate.getBlock() != this) {
         ...
         return !(held.getItem() instanceof ItemGun);
      }
   }
   ...
   public void toggleDoor(World worldIn, BlockPos pos, boolean open) {
      ...
      if (iblockstate.getBlock() == this) {
         BlockPos blockpos = iblockstate.getValue(HALF) == BlockGameDoor.EnumDoorHalf.LOWER ? pos : pos.down();
   ...
   public void neighborChanged(IBlockState state, World worldIn, BlockPos pos, Block blockIn, BlockPos fromPos) {
      if (state.getValue(HALF) == BlockGameDoor.EnumDoorHalf.UPPER) {
   ...
   public void onBlockHarvested(World worldIn, BlockPos pos, IBlockState state, EntityPlayer player) {
      BlockPos blockpos = pos.down();
      BlockPos blockpos1 = pos.up();
      if (player.capabilities.isCreativeMode && state.getValue(HALF) == BlockGameDoor.EnumDoorHalf.UPPER && worldIn.getBlockState(blockpos).getBlock() == this) {
         worldIn.setBlockToAir(blockpos);
      }

      if (state.getValue(HALF) == BlockGameDoor.EnumDoorHalf.LOWER && worldIn.getBlockState(blockpos1).getBlock() == this) {
   ...
   public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
      ...
      if (state.getValue(HALF) == BlockGameDoor.EnumDoorHalf.LOWER) {
   ...
   public IBlockState withRotation(IBlockState state, Rotation rot) {
      return state.getValue(HALF) != BlockGameDoor.EnumDoorHalf.LOWER ? state : state.withProperty(FACING, rot.rotate((EnumFacing)state.getValue(FACING)));
   }
   ...
   public IBlockState getStateFromMeta(int meta) {
      return (meta & 8) > 0 ? this.getDefaultState().withProperty(HALF, BlockGameDoor.EnumDoorHalf.UPPER).withProperty(HINGE, (meta & 1) > 0 ? BlockGameDoor.EnumHingePosition.RIGHT : BlockGameDoor.EnumHingePosition.LEFT).withProperty(POWERED, (meta & 2) > 0) : this.getDefaultState().withProperty(HALF, BlockGameDoor.EnumDoorHalf.LOWER).withProperty(FACING, EnumFacing.getHorizontal(meta & 3).rotateYCCW()).withProperty(OPEN, (meta & 4) > 0);
   }
   ...
   public int getMetaFromState(IBlockState state) {
      ...
      if (state.getValue(HALF) == BlockGameDoor.EnumDoorHalf.UPPER) {
         i = i | 8;
         if (state.getValue(HINGE) == BlockGameDoor.EnumHingePosition.RIGHT) {
            i |= 1;
         }
   ...
   static {
      ...
      HINGE = PropertyEnum.create("hinge", BlockGameDoor.EnumHingePosition.class);
      POWERED = PropertyBool.create("powered");
      HALF = PropertyEnum.create("half", BlockGameDoor.EnumDoorHalf.class);
