...
public class ItemPipebomb extends ItemBase {
   public ItemPipebomb(String name, ItemBase.EnumItemType type) {
      super(name, type);
      this.maxStackSize = 1;
   }
   ...
   public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
      ...
      if (!playerIn.isPotionActive(InitPotions.incapacitated) && !Left2MineUtilities.isPinnedPotionActive(playerIn)) {
         ...
         stats.setPipebombsUsed(stats.getPipebombsUsed() + 1);
         if (!worldIn.isRemote) {
            Left2MinePacket.INSTANCE.sendToAll(new StatsMessage(StatsEnum.PIPEBOMBSUSED, stats.getPipebombsUsed(), playerIn));
         }

         if (!worldIn.isRemote) {
            EntityPipebomb pipebomb = new EntityPipebomb(worldIn, (EntityPlayerMP)playerIn);
            playerIn.world.spawnEntity(pipebomb);
         }
