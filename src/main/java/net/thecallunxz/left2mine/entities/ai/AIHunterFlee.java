...
public class AIHunterFlee<T extends Entity> extends EntityAIBase {
   ...
   public AIHunterFlee(EntityCreature entityIn, Class<T> classToAvoidIn, float avoidDistanceIn, double farSpeedIn, double nearSpeedIn) {
      this(entityIn, classToAvoidIn, Predicates.alwaysTrue(), avoidDistanceIn, farSpeedIn, nearSpeedIn);
   }

   public AIHunterFlee(EntityCreature entityIn, Class<T> classToAvoidIn, Predicate<? super T> avoidTargetSelectorIn, float avoidDistanceIn, double farSpeedIn, double nearSpeedIn) {
      this.canBeSeenSelector = new Predicate<Entity>() {
         public boolean apply(@Nullable Entity p_apply_1_) {
            return p_apply_1_.isEntityAlive() && !AIHunterFlee.this.entity.isOnSameTeam(p_apply_1_);
         }
      };
   ...
   public boolean shouldExecute() {
      ...
      if (!(this.entity instanceof EntityHunter) || !((EntityHunter)this.entity).isPinned() && !((EntityHunter)this.entity).isPouncing() && !((EntityHunter)this.entity).isSneaking() && ((EntityHunter)this.entity).getPounceTicks() >= -10 && !Left2MineUtilities.canPlayersSeeEntity(this.entity.world, (EntityHunter)this.entity) && this.entity.ticksExisted <= 600) {
