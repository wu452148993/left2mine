...
public class AISmokerFlee<T extends Entity> extends EntityAIBase {
   ...
   public AISmokerFlee(EntityCreature entityIn, Class<T> classToAvoidIn, float avoidDistanceIn, double farSpeedIn, double nearSpeedIn) {
      this(entityIn, classToAvoidIn, Predicates.alwaysTrue(), avoidDistanceIn, farSpeedIn, nearSpeedIn);
   }

   public AISmokerFlee(EntityCreature entityIn, Class<T> classToAvoidIn, Predicate<? super T> avoidTargetSelectorIn, float avoidDistanceIn, double farSpeedIn, double nearSpeedIn) {
      this.canBeSeenSelector = new Predicate<Entity>() {
         public boolean apply(@Nullable Entity p_apply_1_) {
            return p_apply_1_.isEntityAlive() && !AISmokerFlee.this.entity.isOnSameTeam(p_apply_1_);
         }
      };
   ...
   public boolean shouldExecute() {
      ...
      if (this.entity instanceof EntitySmoker) {
         if (((EntitySmoker)this.entity).isSmoked() || ((EntitySmoker)this.entity).getSeenTicks() > 5 || this.entity.ticksExisted > 400 && ((EntitySmoker)this.entity).getRechargeTime() < this.entity.world.getTotalWorldTime()) {
            return false;
         }

         if (((EntitySmoker)this.entity).getRechargeTime() > this.entity.world.getTotalWorldTime()) {
            this.avoidDistance *= 2.5F;
         }
      }
   ...
   public boolean shouldContinueExecuting() {
      if (!(this.entity instanceof EntitySmoker) || !((EntitySmoker)this.entity).isSmoked() && (this.entity.ticksExisted <= 400 || ((EntitySmoker)this.entity).getRechargeTime() >= this.entity.world.getTotalWorldTime())) {
