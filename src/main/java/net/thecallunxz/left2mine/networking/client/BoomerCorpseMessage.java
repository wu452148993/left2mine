...
public class BoomerCorpseMessage implements IMessage {
   ...
   public BoomerCorpseMessage() {
   }

   public BoomerCorpseMessage(double origX, double origY, double origZ, Entity ent, float power, float damage, long timeKilled) {
      this.ent = ent;
      this.power = power;
      this.damage = damage;
   ...
   public static class Handler implements IMessageHandler<BoomerCorpseMessage, IMessage> {
      public IMessage onMessage(final BoomerCorpseMessage message, MessageContext ctx) {
         ...
         mainThread.addScheduledTask(new Runnable() {
            public void run() {
               ...
               if (newEnt == null) {
                  ...
                  newEnt = new EntityBoomer(Minecraft.getMinecraft().world);
                  ((Entity)newEnt).setPositionAndRotation(entX, d3, d4, message.entYaw, message.entPitch);
               }

               ((Entity)newEnt).world.playSound(Minecraft.getMinecraft().player, ((Entity)newEnt).getPosition(), InitSounds.boomer_explode, SoundCategory.HOSTILE, 8.0F, 1.0F);
               ...
               for(int j = 0; j < particleCount; ++j) {
                  ...
                  d4 /= d6;
                  d5 /= d6;
                  Particle origBile = new EntityBoomerFX(((Entity)newEnt).world, ((Entity)newEnt).posX, ((Entity)newEnt).posY + 1.5D, ((Entity)newEnt).posZ, (double)((Entity)newEnt).world.rand.nextFloat() * (d3 / 1.5D), (double)((Entity)newEnt).world.rand.nextFloat() * (d4 / 1.5D), (double)((Entity)newEnt).world.rand.nextFloat() * (d5 / 1.5D), 8.0D);
