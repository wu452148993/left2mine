...
public class RenderMolotov extends Render<EntityMolotov> {
   public static final RenderMolotov.Factory FACTORY = new RenderMolotov.Factory();

   public RenderMolotov(RenderManager renderManager) {
      super(renderManager);
   ...
   public void doRender(EntityMolotov entity, double x, double y, double z, float entityYaw, float partialTicks) {
      ...
      IBakedModel model = Minecraft.getMinecraft().getRenderItem().getItemModelMesher().getItemModel(new ItemStack(InitItems.molotov));
   ...
   public static class Factory implements IRenderFactory<EntityMolotov> {
      public Render<? super EntityMolotov> createRenderFor(RenderManager manager) {
         return new RenderMolotov(manager);
      }
   }
}
